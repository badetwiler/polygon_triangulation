 ...
 * 
 */
Shape.prototype.mvPushMatrix = function(){
 ...
 * 
 */
Shape.prototype.mvPopMatrix = function(){
	if(this.mvMatrixStack.length == 0){
		throw "Invalid pop matrix in Shape";
	}
 ...
 * 
 */
Shape.prototype.alignWithCamera = function(globalMVmatrix){
	mat4.set(globalMVmatrix,this.mvMatrix);
}
...
/*
  ...
  */
	...
	if(!doneOnce)
    	console.log("error code after drawElements: " + gl.getError().toString(16));
...
/**
 * @param {Array[float]} p 2D array (for now)
 */
Shell.prototype.addPoint = function(p){
	this.vertexes.push(p[0]);
	this.vertexes.push(p[1]);
	this.vertexes.push(p[2]|| 0.0);
	
	this.normalData.push(0.0);
	this.normalData.push(0.0);
	
	//will need to be dynamic
	this.normalData.push(-1.0);
	
    Shape.prototype.initVertexPositionBuffer.call(this,this.vertexes,3,this.vertexes.length/3);
    Shape.prototype.initVertexNormalBuffer.call(this,this.normalData,3,this.normalData.length/3)
 ...
 * 
 */ 
Shell.prototype.deleteLastPoint = function(){
...
Shell.prototype.draw = function(){
	...
	if(this.isDrawingOutline){
	    ...
	    setMatrixUniforms();
	    gl.drawArrays(gl.LINE_STRIP,0,this.vertexes.length/3);
		if(!doneOnce)
	    	console.log("error code after drawElements: " + gl.getError().toString(16));
	}
	else if(this.isExtruded){
